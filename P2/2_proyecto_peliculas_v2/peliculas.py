...
def borrarPeliculas():
    ...
    resp = input("¿Deseas quitar o borrar todas las caracteristicas de la pelicula? (Si/No): ").lower().strip()
    if resp == "si":
        pelicula.clear()
        input("\n\t\t::: ¡LA OPERACIÓN SE REALIZÓ CON ÉXITO! :::")
...
def modificarCaracteristicaPeliculas():
    ...
    if len(pelicula) > 0:
        for clave in pelicula:
            ...
            resp=input("\n\tDeseas modificar la cacteristica de {clave}?(Si/No)").lower().strip()
            if resp=="si":
                pelicula[clave]=input("Ingresa el nuevo valor de la caracteristica: {clave}").upper().strip()
                print("\n\t\t::: ¡LA OPERACIÓN SE REALIZÓ CON ÉXITO! :::")
        else:
